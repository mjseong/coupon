...
- 100억개 쿠폰코드를 관리하기 위해서는 DB sharding을 고려해야 합니다. 구현하지는 못했지만 아래와 같은 전략으로 접근해 볼 것 같습니다.
   ...
   * 위 내용을 토대로 구현된 DB샤딩은 검색시 exprieDate을 이용하여 Range 검색을 해야합니다. 현재 쿠폰코드를 uuid로만 구현했기 때문에 exprieDate에 대한 정보를 포함해야 합니다. 추후에 쿠폰코드 생성기를 만들어 적용해야할 것 같습니다.
